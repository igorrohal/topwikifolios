...
import static eu.irohal.topwikifolios.Configuration.WIKIFOLIOS_TOP_PERFORMANCE_BASE;
...
@Component
public class TopPerformanceFetcher {
...
//            WIKIFOLIOS_TOP_PERFORMANCE_BASE + "&startValue=96");
    ...
    public void fetch() {
        log.info("TopPerformanceFetcher triggered at " + LocalTime.now());
        ...
        final String csvContents = allWikifolios.stream()
                .map(w -> wikifolioSerializer.topPerformance(w, CSV_DELIMITER))
                .reduce(wikifolioSerializer.topPerformanceHeader(CSV_DELIMITER), (i, ws) -> i += "\n" + ws);
